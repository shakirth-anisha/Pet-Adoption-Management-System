...
def handle_manage_applications(request):
    """Handle manage applications page with filtering"""
    status_filter = request.args.get('status', 'all')
    
    # Build query with optional filter
    query = """
        SELECT a.adopt_app_id, a.status, a.reason, a.date,
    ...
    # Get list of all shelter workers for the dropdown
    workers_query = """
        SELECT sw.worker_id, u.name AS worker_name, s.name AS shelter_name
...
def approve_application(app_id, worker_id):
    ...
    try:
        # Call ApproveApplication procedure
...
def reject_application(app_id, reason, worker_id=None):
    """Reject an adoption application using stored procedure"""
    try:
        # Call RejectApplication procedure (only takes app_id and reason)
        run_procedure("RejectApplication", (app_id, reason))
        
        # Update the approved_by field separately if worker_id is provided
        if worker_id:
            update_query = """
                UPDATE AdoptionApplication 
                SET approved_by = %s 
                WHERE adopt_app_id = %s
            """
            run_query(update_query, (worker_id, app_id))
