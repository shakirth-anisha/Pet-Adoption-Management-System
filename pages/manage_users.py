...
from utils.db import run_query, get_sql_role

def handle_manage_users(request):
    """Admin page to manage users and their roles"""
    message = None
    alert_class = "info"
    sql_role = get_sql_role(session.get('user_role'))
    ...
    if request.method == "POST":
        ...
        if action == "update_role" and user_id and new_role:
            try:
                ...
                run_query(query, (new_role, user_id), sql_role=sql_role)
    ...
    try:
        users = run_query(query, tuple(params), fetch=True, sql_role=sql_role) or []
    except Exception as e:
        users = []
        if not message:
